...
import serial
#from magnet_calibrate import magnet_calibrate
import spidev 
import picamera
import sys
...
cycle_time = 0.7                 #機体が1回転する時間　その場で20回転くらいで平均
preset_altiude = 5               #着地高度設定
preset_altiude2 = 6              #上昇高度設定
preset_goal_distanc = 9.0        #誘導方法切り替え距離
go_time = 5                      #GPS誘導時直進時間
preset_pres =1008                #!ゴール地点気圧
career_time =1.5                 #!電熱線燃焼時間
magnet_calibrate_list = [-60,250] #!地磁気補正データ
...
def go2(start, rmax =100, lmax=100 , t = 1.0):
...
def get_moter_time(Angle) :
    x =  cycle_time
    moter_time = (x/360) * math.fabs(Angle) #回転数式
    return moter_time
