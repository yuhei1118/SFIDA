...
cycle_time = 1.1                  #機体が1回転する時間
preset_altiude = 10                #着地高度設定
#preset_altiude2 = 6               #上昇高度設定
preset_goal_distanc = 1.5          #誘導方法切り替え距離
go_time = 10                       #GPS誘導時直進時間
#preset_pres = 882.61              #ゴール地点気圧 
career_time = 4                  #!電熱線燃焼時間
magnet_calibrate_list = [-10,146] #!地磁気補正データ
tem = 33                          #競技地点温度[℃]

LEFT_TOP_M1 = 22 # lt      #モーター・電熱線・通信電源ピンを割り当て
LEFT_TOP_M2 = 27
RIGHT_TOP_M1 = 17 # rt
RIGHT_TOP_M2 = 23
CAREER_CUT = 25
...
def get_altitude(pf) :#戻り値あり(h)　高度(altitude)を取得 →気圧を代入して、それに対する高度が帰ってくる　pf・・・get_altitudeの変数
    p0    = 1013.25
    p0fix = pf
    p1    = float(read_pres()) 
    t1    = float(read_temp())
...
#MPU9250 #修正済（2023/07/06）bus_number = 1,bus = smbus.SMBus(bus_number)は一回だけで良い
bus_number = 1
bus = smbus.SMBus(bus_number)
...
def moter_setup():
    ...
    #GPIOの周波数を設定(Hz) 増やすとスムーズに
    pi.set_PWM_frequency(LEFT_TOP_M1, 100)
    pi.set_PWM_frequency(LEFT_TOP_M2, 100)
    pi.set_PWM_frequency(RIGHT_TOP_M1, 100)
    pi.set_PWM_frequency(RIGHT_TOP_M2, 100)
    #GPIOの範囲を設定　
...
def stop(): #duty比(0=off,50=1/2on,100=full on) 大きくするとモーターの回転数大
...
def gostop(start, max = 100.0, t = 1.0) :
    s = max / t / 10.0
    for duty in drange2(max, 0.0, s): #dutyをmaxから0.0までs刻みで変化させる
...
def go2(start, rmax =100, lmax=100 , t = 1.0):#m++.pyで調べたモーターの回転数ここに入れる
...
def godiststop(dist):
    pi.set_PWM_dutycycle(LEFT_TOP_M2, 0)
    pi.set_PWM_dutycycle(RIGHT_TOP_M2, 0)
    pi.set_PWM_dutycycle(LEFT_TOP_M1, dist)
    pi.set_PWM_dutycycle(RIGHT_TOP_M1, dist)
...
def left(start, max = 100.0, t = 1.0, rate = 1.0): #左回転
    if rate >= 0:
        r = max
        l = rate * max
        pi.set_PWM_dutycycle(LEFT_TOP_M2, 0)
...
def right(start, max = 100.0, t = 1.0, rate = 1.0): #右回転
    if rate >= 0:
        l = max
        r = rate * max
        pi.set_PWM_dutycycle(LEFT_TOP_M2, 0)
...
def career_cut(t=3) :
    career_time = t 
    print("点火")
    pi.write(CAREER_CUT , 1 )#電気側にCAREER_CUT=1を送信する
    time.sleep(career_time)
    pi.write(CAREER_CUT , 0 )#電気側にCAREER_CUT=0を送信する
    time.sleep(1)
    print("点火")
...
def get_sequence_count() :
    with open("sequence_count.txt" , "r") as sf :#ファイル作成　sfにopenで作成したファイルを代入する　作成されたファイル名がsequence_count.txt　rなので読み取り専用
        sequence_count =sf.read()#シーケンスカウントの変数にsfからreadした読んだ値を代入する
    print("get_seqence")#シーケンスカウントを受け取ったことを報告
    return int(sequence_count)#sfファイルから読み取ったシーケンスカウントの値を返す

#シーケンス保存
def write_sequence_count(sequence_count) :#シーケンスカウントに追記述する関数
...
#通信開始用フラグ保存
def write_im920flag(flag) :#フラグに追記述する関数
    f = str(flag)#フラグを扱う変数f
    with open("im_flag.txt" , "w") as sf :
        flag =sf.writelines(f)#フラグ変数に記述
        
    return 0

def get_preset_pres() :#preset_presを返す→pf.readより取得した値
    with open("preset_pres.txt" , "r") as pf :
        preset_pres =pf.read()#preset_pres・・・pf(preset_pres.txt　)から読み込んだ値
...
#モーター駆動時間設定
def get_moter_time(Angle) :#角度Angleの値に対して算出される
    x =  cycle_time
...
def distancetoobject(TRIG_PIN, ECHO_PIN) :
    num = 0
    times = 3 #繰り返し回数
    while num < times:
        disf = False
        ...
        if calc_distance(TRIG_PIN, ECHO_PIN, 1, v) > dis :
            disf = False
        elif calc_distance(TRIG_PIN, ECHO_PIN, 1, v) < dis :
            disf = True
        time.sleep(1)                          #1秒間待つ
        num += 1
...
def main() :
    ...
    if sequence_count == 0 :
        ...
        while True :
         ...
         print(photoresistor_voltage)
         time.sleep(1) 
         if photoresistor_voltage < 1.85 : #本番の環境に合わせて閾値変える
           print("check2")
           sequence_count = 1
    ...
    if sequence_count == 2 :
        f = 1 #fでカウント（保険）
        while True :
            my_point = get_gps_realtime()#自分の座標を取得
            
            if get_altitude(preset_pres) < preset_altiude :
                
                time.sleep(3)
                print("landing_1")#着陸
                career_cut(career_time)
                sequence_count = 3
                write_sequence_count(sequence_count)
                write_log(sequence_count,gps = get_gps() ,altitude = get_altitude(preset_pres) ,photvolt = 0 ,
            ...
            #if文で着陸判定出ないとき（気圧センサ不具合など）
            elif f > 1000 : #300だと降下途中で自由落下してしまう→多めに取っておく（計算してみる）
                time.sleep(5)
                print("landing_2")
                career_cut(career_time)
                sequence_count = 3
    ...
    #遠距離誘導シーケンス
    if sequence_count == 4 : 
        stack_count = 0#おそらくstack_count=0で[スタックしていない]という意味
        ...
        while True :
            ...
            elif stack_count == 3 :#スタック=3　スタックした場合
                write_log(sequence_count,gps = get_gps() ,altitude = get_altitude(preset_pres) ,photvolt = 0 ,
