...
def main():
    ...
    f0 = open('kasokudolog.txt','a', encoding='UTF-8')
    f0.writelines("")
    f0.writelines("\n")
    f0.close()
    f0 = open('BME.txt','a', encoding='UTF-8')
    f0.writelines("")
    f0.writelines("\n")
    f0.close()
    
    while log_count <= 200 :
        f1 = open('kasokudolog.txt', 'a', encoding='UTF-8')
        f1.writelines(format(sensor.acceleration))
        f1.writelines("\n")
        f1.close()
        f2 = open('BME.txt', 'a', encoding='UTF-8')
        f2.writelines(str(readtemp()))
        f2.writelines(",")
        f2.writelines(str(readpres()))
        f2.writelines(",")
        f2.writelines(str(altitude()))
        f2.writelines("\n")
        f2.close()
        
        print("{}".format(sensor.acceleration))
        print_list  =["気温",str(readtemp()), "気圧",str(readpres()), "高度",str(altitude())]
        print(print_list)
